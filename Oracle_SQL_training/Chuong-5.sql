...
SELECT *
FROM EMP E
WHERE E.SAL = (SELECT MAX(SAL) FROM EMP WHERE EMP.JOB = E.JOB);
...
SELECT *
FROM EMP E
WHERE E.SAL > (SELECT AVG(EMP.SAL) FROM EMP WHERE EMP.DEPTNO = E.DEPTNO);
...
FROM EMP E,
     ...
     SALGRADE
WHERE E.DEPTNO = M.DEPTNO
  AND M.SAL BETWEEN SALGRADE.LOSAL AND SALGRADE.HISAL
  AND M.JOB = 'MANAGER';